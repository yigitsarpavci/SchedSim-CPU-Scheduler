...
\begin{enumerate}
    ...
    \item Null-terminate the input buffer and identify the end-of-input (handling both LF and CRLF line endings).
    ...
    \item Write the accumulated output buffer to \texttt{stdout}.
...
\begin{itemize}
    ...
    \item \texttt{output\_buf} (4096 bytes) -- the timeline output buffer. Characters are appended sequentially.
    \item \texttt{output\_len} (8 bytes) -- the current number of characters in \texttt{output\_buf}, used as the write index.
    \item \texttt{input\_buf} (257 bytes) -- up to 256 raw input bytes from stdin plus one reserved byte for the null terminator.
\end{itemize}
...
\begin{figure}[h]
    ...
    \caption{Hand-drawn schematic representing memory structure and circular round-robin queue mechanics.}
...
\begin{itemize}
    ...
    \item \textbf{append\_output}: A small helper that appends a single character to \texttt{output\_buf} and increments \texttt{output\_len}. It preserves the temporary registers it uses.
    \item \textbf{do\_output}: Writes \texttt{output\_buf} to stdout via \texttt{sys\_write}, then exits via \texttt{sys\_exit}.
\end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item The automated grading flow (\texttt{make testgrade}), which runs all provided test cases and then invokes the Python grader for line-by-line comparison.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Zero-burst processes}: Processes whose burst time is 0 are ignored during parsing. They do not enter the scheduler and do not produce process output or idle padding.
...
\begin{itemize}
    ...
    \item \textbf{Static vs.\ dynamic design}: All buffers are statically allocated with fixed maximum sizes (10 processes, 4096-byte output). This avoids dynamic memory allocation complexity entirely, which is acceptable given the problem constraints.
\end{itemize}


\section*{Technical Decisions for Robustness}
To ensure stability and correctness across all possible edge cases, several critical design decisions were made:
\begin{itemize}
    \item \textbf{Cross-platform Line Endings}: The parser explicitly handles both LF (\texttt{\textbackslash n}) and CRLF (\texttt{\textbackslash r\textbackslash n}) line endings, ensuring compatibility whether the input is generated on Linux, Windows, or macOS.
    \item \textbf{Large-Scale Output Support}: The output buffer was increased to 4096 bytes to accommodate high burst times (e.g., 10 processes with maximum burst values) without risk of overflow.
    \item \textbf{Zero-Burst Filtering}: In accordance with TA forum clarifications, processes with a burst time of zero are entirely skipped during parsing, preventing unnecessary idle cycles or timeline anomalies.
    \item \textbf{Deterministic Tie-breaking}: For all preemptive algorithms (SRTF, PF), a strict multi-level comparison cascade ensures that ties are always broken by the original input order (first-found wins).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Possible improvements}: A jump table could replace the comparison chain for algorithm dispatch. The sorting phases in FCFS and SJF could be replaced with selection-based approaches that avoid the stack-allocated order array. From a syntactic perspective, using assembler macros (\texttt{.macro}) for repetitive register saving/restoring blocks and defining system call numbers as constants (\texttt{.equ}) would further enhance code cleanliness and maintainability. Additionally, more extensive error handling (e.g., for malformed input) could be added, though it is not required by the specification.
\end{itemize}
