...
\begin{enumerate}
    \item Read up to 255 bytes from \texttt{stdin} via the \texttt{sys\_read} syscall.
...
\begin{itemize}
    ...
    \item \texttt{input\_buf} (256 bytes) -- the raw input read from stdin, with one extra byte reserved for the null terminator.
\end{itemize}
